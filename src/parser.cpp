...
/*				if (i + pf_dist_s1 < data_size && v_kmers_data_short[i + pf_dist_s1].first >= 0)
			...
			if (h != HT_FAIL)
			{
				...
				for (int j = j_start; j < bucket_size && bucket[j] < ref_pred_pos + params.close_dist; ++j)
				{
					...
					if (matching_len == params.min_match_len && abs(pos - ref_pred_pos) >= params.close_dist / 64)
						matching_len = 0;
					if (matching_len == params.min_match_len + 1 && abs(pos - ref_pred_pos) >= params.close_dist / 8)
						matching_len = 0;
