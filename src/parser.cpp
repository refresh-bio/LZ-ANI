...
bool CParser::prepare_reference(const seq_view ref_view, uint32_t n_seqs)
{
	...
	append(seq_ref, params.max_dist_in_ref, code_N_ref);
	append(seq_ref, params.max_dist_in_ref, code_N_ref);
	...
	prepare_kmers(v_kmers_ref_short, seq_ref, params.min_seed_len, true);
	prepare_ht_long();
	prepare_ht_short();
...
bool CParser::prepare_data(const seq_view data_view, uint32_t n_seqs)
{
	...
	append(seq_data, params.max_dist_in_ref, code_N_seq);

	n_data_seqs = n_seqs;

	prepare_kmers(v_kmers_data_short, seq_data, params.min_seed_len, true);
...
{
	uint32_t ht_size = 1u << (2 * params.min_seed_len);
	...
	for (int i = 0; i < n_kmers; ++i)
	{
		...
		if (v_kmers_ref_short[i].first >= 0)
		{
			ht_short[ht_short_desc[v_kmers_ref_short[i].first].first] = (int) v_kmers_ref_short[i].second;
...
/*	if (region_len < params.min_region_len)
	...
	*/
	if ((region_len - no_miss) / region_len < 0.8)
...
void CParser::parse()
{
	...
	for (i = 0; i + params.min_seed_len < data_size;)
	{
		int best_pos = 0;
		int best_len = 0;
		int h;
...
/*				if (i + pf_dist_s1 < data_size && v_kmers_data_short[i + pf_dist_s1].first >= 0)
			...
			if (h != HT_FAIL)
			{
				...
				int j_start = (int) (lower_bound(bucket, bucket + bucket_size, ref_pred_pos - cur_lit_run_len) - bucket);

				for (int j = j_start; j < bucket_size && bucket[j] < ref_pred_pos + params.max_dist_in_ref; ++j)
				{
					auto pos = bucket[j];
					int matching_len = equal_len(pos, i, params.min_seed_len);

//					if (matching_len == params.min_seed_len && abs(pos - ref_pred_pos) >= params.max_dist_in_ref / 16)
//					if (matching_len == params.min_seed_len && cur_lit_run_len > params.max_dist_in_query / 8)
//						matching_len = 0;
			...
			if (best_anchor_pos)
			{
				...
				else
				{
					...
					double anchor_prob = ipow(1 - prob_len(best_anchor_len), (int) (2 * (seq_ref.size() + 1 - best_anchor_len)));
					double close_prob = ipow(1 - prob_len(best_len), cur_lit_run_len + params.max_dist_in_ref + 1 - best_len);
		...
		if (best_len >= params.min_seed_len)
		{
			flag_t flag = flag_t::match_distant;

			if (ref_pred_pos >= 0 && abs(best_pos - ref_pred_pos) <= params.max_dist_in_ref)
			{
...
//						prev_region_start = -1;
		...
		if (cur_lit_run_len > params.max_dist_in_query)
