...
class CParser
{
	...
	int ht_short_mask = 0;
	uint32_t ht_long_size = 0;
	uint32_t ht_long_mask = 0;
...
public:
	CParser(const CParams &params) :
		params(params)
	{
		est_len_correction = params.min_seed_len - (16 - (params.min_anchor_len - params.min_seed_len));
	}
